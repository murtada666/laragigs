<x-layout>
    <x-card class="p-10 max-w-lg mx-auto mt-24">
                    ...
                    {{-- enctype is necessary when dealing with file input! --}}
                    <form action="/listings" method="POST" enctype="multipart/form-data">
                        
                        {{-- A directive used when using a POST request
                            It prevents cross-site scripting attacks
                            so it stops others to submit to my endpoint --}}
                        @csrf

                        <div class="mb-6">
                            <label for="company" class="inline-block text-lg mb-2">Company Name</label>
                            {{-- We can use (old) helper to retrieve old data when an error occur (keep old data in form) --}}
                            <input type="text" class="border border-gray-200 rounded p-2 w-full" name="company" value="{{old('company')}}"/>
                            {{-- 
                                - We can use error directive because we used validate.
                                - Error name should match the input name!.
                                --}}
                            @error('company')
                        ...
                        <div class="mb-6">
                            ...
                            <input
                                type="text" class="border border-gray-200 rounded p-2 w-full" name="title" placeholder="Example: Senior Laravel Developer" value="{{old('title')}}"/>
